    ...
    //criar todas as 9 peças virtualmente
    let pecaVirtual = [
        document.createElement("p"), // essa peçla não é utilizada
    ...
    btn[i].addEventListener("click", function(e){
        ...
        if(jogoAcabado === 0){
            if(valorBtn[i] === 0){
                //o mig serve para receber se o valor é X ou O e transformar isso em numero ou 1 ou 2
               mig = contarRodada();
                if (mig === "X"){
                    valorBtn[i] = 1;
                }else{
                ...
                pecaVirtual[btn[i].value].innerHTML = quemJoga;

                // o LOCAL  que vai ser colocado
                const insert = document.querySelector("#a"+btn[i].value);
                insert.appendChild(pecaVirtual[btn[i].value]);
           }else{
...
//            insert = document.querySelector("#a"+btn[0].value);
////          insert = document.querySelector("#a"+btn[i].value);
//            insert.appendChild(jogador);

        for(let i=0; i<btn.length; i++){
            pecaVirtual[btn[i].value].innerHTML = " ";
        }
    });
